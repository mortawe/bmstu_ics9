...
void print_part(tree node) {
    ...
    switch (TREE_CODE(node)) {
        case IDENTIFIER_NODE:
            ...
            break; // names of functions (calls)
        case VAR_DECL:
        ...
        case SSA_NAME:  // arguments _x
            if (SSA_NAME_IDENTIFIER(node)) {
                print_part(SSA_NAME_IDENTIFIER(node));
            }
...
const char *print_symbol_code(enum tree_code code) {
    switch (code) {
        case TRUTH_OR_EXPR:
...
void print_stmt(gimple stmt) {
    ...
    switch (gimple_code(stmt)) {
        ...
        case GIMPLE_CALL:
            ...
            print_part(DECL_NAME(TREE_OPERAND(gimple_call_fn(stmt), 0)));
            printf("(");
            int i;
            for (i = 0; i < gimple_call_num_args(stmt); i++) {
                print_part(gimple_call_arg(stmt, i));
        ...
        case GIMPLE_ASSIGN:
            ...
            switch (gimple_num_ops(stmt)) {
                ...
                case 3:
                    ...
                    printf("%s ", print_symbol_code(gimple_assign_rhs_code(stmt)));
                    print_part(rhs2);
                    break;
            }
            break;
        case GIMPLE_RETURN:
            ...
            print_part(gimple_return_retval(stmt));
            printf("\n");
            break;
        default:
            break;
...
perform_tree_ssa_dce(bool aggressive) {
    ...
    if (aggressive) {
        ...
        cd = new
        control_dependences(create_edge_list());
